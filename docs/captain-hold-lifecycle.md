...
A local-only landing's selected-branch note is written by `tasks-axi done --note` as the last of the row's indented body lines rather than into the row title, so that final line is the only place an arbitrary branch is read from, and the landing is published carrying its recorded note.
The title is still parsed for the historical `local main` suffix alone, so ordinary title prose that happens to end in the word "local" followed by another word is never mined as a landing artifact.
A body that carries a captain resolution record is the captain's own prose and is never mined for that note, so a decision containing a local-target phrase does not become a delivery artifact.
