...
#                the configured merge authority approves, then firstmate uses the
#                guarded path to fast-forward the selected local target branch
...
case "$MODE" in
  ...
  local-only)
    SETUP2=""
    RULE1="1. Never push to any remote and never open a PR. Work only on your \`fm/$ID\` branch; firstmate handles the guarded merge into the selected local target branch."
    ;;
  *)  # no-mistakes
    SETUP2="
